...
// 串口初始化
std::unique_ptr<serial::Serial> referee_serial = std::make_unique<serial::Serial>("/dev/sentry_serial", 115200U, serial::Timeout::simpleTimeout(50U),
...
int main(int argc, char **argv)
{
	...
	while (true)
	{
		...
		{
			try {
			referee_serial = std::make_unique<serial::Serial>("/dev/sentry_serial", 115200U, serial::Timeout::simpleTimeout(50U),
                   serial::eightbits, serial::parity_none, serial::stopbits_one,
                   serial::flowcontrol_none);
