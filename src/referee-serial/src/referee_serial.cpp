...
public:
  ...
  void Tranport(const char *data) {
    frame_header_t head = {};
    memcpy(&head, data, sizeof(frame_header_t));
    switch (head.cmdid) {
    case 0x0001: {
      ...
      if ((data[8] & 0xf0) == 0x40) {
        ...
        msg.data = (float)(*(uint16_t *)(&data[9]));
      }
    ...
    case 0x0003: {
      auto msg = std_msgs::msg::Float32();
      if (isred)
        msg.data = (float)(*(uint16_t *)&data[8 + 10]);
      else
        msg.data = (float)(*(uint16_t *)&data[8 + 26]);
      hp_pub_->publish(msg);
      break;
    }
    case 0x0207: {
...
int main(int argc, char **argv) {
  ...
  std::shared_ptr<RefereeTransport> referee =
      std::make_shared<RefereeTransport>("referee/transport", argv[0]);

  while (true) {
    std::string temp = referee_serial.readline();
    referee->Tranport(temp.c_str());
  }
  //   try {
  //     // 初始化节点
  //     rclcpp::init(argc, argv);
